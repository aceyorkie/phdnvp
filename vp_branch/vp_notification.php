...
if (isset($_GET['event_id'])) {
    ...
    if ($event_id !== null && $event_id > 0) {
        ...
        header("Location: /PH_DN_VP_Dashboard/vp_branch/request.php?request_id={$latest_request_id}");
        exit();
    }
}
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "
        <a href='vp_notification.php?event_id={$event_id_display}' class='{$itemClass}'>
            {$badge} {$message}<br>
