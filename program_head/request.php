...
$programheadDb = new mysqli('localhost', 'root', '', 'orgportal');
if ($programheadDb->connect_error) {
  die("Connection failed (programhead): " . $programheadDb->connect_error);
}

$checkSig = $programheadDb->prepare("SELECT signature FROM programhead WHERE id_no = ?");
$checkSig->bind_param("i", $programhead_id);
$checkSig->execute();
$sigResult = $checkSig->get_result()->fetch_assoc();
$programheadSignature = !empty($sigResult['signature']) ? $sigResult['signature'] : null;
$checkSig->close();

$id_no = $programhead_id; 
$programheadSql = "SELECT id_no, name, department, signature FROM programhead WHERE id_no = ?";
$programheadStmt = $programheadDb->prepare($programheadSql);
$programheadStmt->bind_param("i", $id_no);
$programheadStmt->execute();
$programheadResult = $programheadStmt->get_result();
$programhead = $programheadResult->fetch_assoc();
$programheadStmt->close();

$dean_query = "SELECT id_no, name FROM dean LIMIT 1";
$dean_result = $programheadDb->query($dean_query);
$dean = $dean_result->fetch_assoc();

$vp_query = "SELECT id_no, name FROM vp LIMIT 1";
$vp_result = $programheadDb->query($vp_query);
$vp = $vp_result->fetch_assoc();

$osa_query = "SELECT id_no, name FROM osa LIMIT 1";
$osa_result = $programheadDb->query($osa_query);
...
$latestRequestSql = "SELECT request_id FROM signature_flow WHERE role = 'Program Head' AND signatory_id = '$id_no' ORDER BY id DESC LIMIT 1";
...
<html>
<head>
    <title>Program Head Signatory Dashboard</title>
...
<body>
    ...
    <div class="main-container">
        <h2>Welcome Program Head: <?php echo htmlspecialchars($programhead['name']); ?></h2>
    ...
    function loadDocs() {
      ...
      const signatoryId = "<?php echo htmlspecialchars($programhead['id_no']); ?>";
      document.getElementById("form_request_id").value = reqId;
      fetch("view_documents.php?request_id=" + reqId + "&signatory_id=" + signatoryId)
        .then(res => res.json())
        .then(data => {
    ...
    document.getElementById("submitBtn").addEventListener("click", async () => {
      ...
      for (const [path, pageCanvases] of Object.entries(annotationsStore)) {
        ...
        formData.append("role", "Program Head");
  ...
  <script>
    ...
    const hasSignature = <?php echo (!empty($programhead['signature'])) ? 'true' : 'false'; ?>; 
