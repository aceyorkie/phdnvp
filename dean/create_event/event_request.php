...
$dean_id = $_SESSION['id_no'];

$deanDb = new mysqli('localhost', 'root', '', 'orgportal');
if ($deanDb->connect_error) {
  die("Connection failed (dean): " . $deanDb->connect_error);
}

$checkSig = $deanDb->prepare("SELECT signature FROM dean WHERE id_no = ?");
$checkSig->bind_param("i", $dean_id);
$checkSig->execute();
$sigResult = $checkSig->get_result()->fetch_assoc();
$deanSignature = !empty($sigResult['signature']) ? $sigResult['signature'] : null;
$checkSig->close();

$id_no = $dean_id; 
$deanSql = "SELECT id_no, name, signature FROM dean WHERE id_no = ?";
$deanStmt = $deanDb->prepare($deanSql);
$deanStmt->bind_param("i", $id_no);
$deanStmt->execute();
...
<body>
  ...
  <div class="main-container">
    ...
    <div class="Documents-box">
      ...
      <input type="hidden" id="current_role" value="Dean">
      ...
      <form method="POST" action="submit_event_drawn_documents.php" enctype="multipart/form-data" id="adviserForm">
        ...
        <div class="select-approvers-wrapper">
          ...
          <div class="select-approvers">
            ...
            <!-- dean -->
            <div class="signatory-card">
              <div class="signature-placeholder" id="dean-signature-box" onclick="triggerFileInput('dean')">
                <?php if (!empty($dean['signature'])): ?>
                  <img src="<?php echo htmlspecialchars($dean['signature']); ?>" alt="Signature" style="width:100px; height:auto; display:block; margin:auto;">
                <?php else: ?>
                  <span>Pending</span>
                <?php endif; ?>
              ...
              <div class="sign-info" style="width:100%;">
                <input type="text" 
                      ...
                      autocomplete="off"
                      value="<?php echo htmlspecialchars($dean['name']); ?>"
                      readonly>
                <input type="hidden" 
                      id="dean_id" 
                      name="dean_id" 
                      value="<?php echo htmlspecialchars($dean['id_no']); ?>">
  ...
  <script>
    ...
    function loadDocs() {
      ...
      const signatoryId = "<?php echo htmlspecialchars($dean['id_no']); ?>";
    ...
    for (const [path, pageCanvases] of Object.entries(annotationsStore)) {
      try {
        ...
        console.log("Sending FormData with:", { requestId, eventId, role: "Dean" });
      ...
      showSuccessToast(`Dean approval completed successfully.`);
      loadDocs();
      document.getElementById("docPreview").innerHTML = "";
    });
  ...
  <script>
    ...
    const hasSignature = <?php echo (!empty($dean['signature'])) ? 'true' : 'false'; ?>; 
