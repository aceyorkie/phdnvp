...
if (!empty($event_id) && $event_id !== "N/A") {
    ...
    ");
    $updateStmt->bind_param("ii", $dean_id, $event_id);
    $updateStmt->execute();
    $updateStmt->close();

    header("Location: /PH_DN_VP_Dashboard/dean/create_event/event_request.php?event_id={$event_id}");
    exit();
}
...
if (!empty($org_id) && $org_id !== "N/A") {
    ...
    ");
    $updateOrg->bind_param("ii", $dean_id, $org_id);
    $updateOrg->execute();
    $updateOrg->close();

    header("Location: /PH_DN_VP_Dashboard/dean/request.php?org_id={$org_id}");
    exit();
}
...
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $link = !empty($event_id)
            ? "/PH_DN_VP_Dashboard/dean/dean_notification.php?event_id={$event_id}"
            : "/PH_DN_VP_Dashboard/dean/dean_notification.php?org_id={$org_id}";
